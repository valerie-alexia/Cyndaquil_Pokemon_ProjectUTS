    ...
    function main() {
        ...
        // 1. MODIFIKASI event listener tombol 'c'
        window.addEventListener("keydown", (e) => {
            if (e.key === 'c' || e.key === 'C') {
                // TIDAK PERLU 'currentTime' LAGI
                body.toggleCrawlState(); // Panggil tanpa parameter
                
                // PENTING: Lakukan hal yang sama untuk semua child
                // HAPUS TANDA KOMENTAR DI BAWAH INI
                body.childs.forEach(child => {
                if (child.toggleCrawlState) {
                    child.toggleCrawlState(); // Panggil juga tanpa parameter
                }
                });
            }
        });
    ...
            head.animate(timeInSeconds);
            ...
            // Panggil juga pada semua child jika mereka punya fungsi animate
            body.childs.forEach(child => {
                if (child.animate) {
