...
export class LegsShape {
    ...
    constructor(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, side = 1) {
        ...
        // Proportions
        var hipCenter = { x: 1.1 * side, y: -3.5, z: 0.01 };
        var legLen = 0.8;
        var legDirY = -1;

        // Colors0.98, 0.94, 0.76
        var legColor = { r: 0.98, g: 0.94, b: 0.7}; //{ 0.98, 0.94, 0.76]

        // 1) Upper Leg (Thigh) - A thick ellipsoid
        this.addEllipsoid(0.9, 1.4, 1, 24, 32, hipCenter, legColor);

        // 2) Lower Leg (Foot/Ankle area)
        var footCenter = {
            x: hipCenter.x,
            y: hipCenter.y + legDirY * (legLen / 2) - 0.8,
            z: hipCenter.z + 0.4
        };
        this.addEllipsoid(0.55, 0.2, 0.5, 20, 28, footCenter, legColor);

        // 3) Claws
        var clawBaseY = footCenter.y - 0.1;
        var spread = 0.25;
        var clawBaseZ = footCenter.z + 0.2;
        ...
        this.addCone(0.09, 0.20, 12, { x: footCenter.x + spread * side, y: clawBaseY, z: clawBaseZ }, legColor, legDirY);
        // semakin besar semakin atas
        LIBS.translateY(this.POSITION_MATRIX, -1.3);
        // --- FIX: Add rotation to the entire leg for better posture ---
        LIBS.rotateY(this.POSITION_MATRIX, 0.1* side); // Splay legs outwards
        LIBS.rotateX(this.POSITION_MATRIX, -0.2);      // Tilt forward
    }
