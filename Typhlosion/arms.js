...
export class ArmShape {
    ...
    constructor(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, side = 1) {
        ...
        // Basic proportions
        var shoulderCenter = { x: 1 * side, y: -0.1, z: 1}; //atur posisi tangan (biar di cream)
        var upperLen = 2.3;
        ...
        var armColor = { r: 0.98, g: 0.94, b: 0.7 };  //{0.98, 0.94, 0.76]
        var clawColor = { r: 0.98, g: 1, b: 0.68 };
        ...
        // 5) Fingers (5 claws, sedikit beda panjang + pipih)
        const wristY = foreCenter.y + armDirY * (foreLen / 2 + 0.00);
        const baseZ  = foreCenter.z + 0.02;

        // sebaran jari relatif terhadap sumbu X lengan (nilai dikali 'side' untuk mirror)
        const offsetsX = [-0.20, -0.10, 0.0, +0.10, +0.20];  // posisi kiri → kanan
        // radius & tinggi masing2 jari (tengah = paling besar)
        const radii    = [0.050, 0.055, 0.060, 0.055, 0.050];
        const heights  = [0.26 , 0.30 , 0.34 , 0.30 , 0.26 ];

        // opsional: kasih sedikit offset Z biar tidak terlalu rata
        const offsetsZ = [-0.005, 0.0, +0.015, 0.0, -0.005];

        for (let i = 0; i < 5; i++) {
        this.addCone(
            radii[i], heights[i], 24,
            {
            x: foreCenter.x + offsetsX[i] * side,   // mirror otomatis
            y: wristY,
            z: baseZ + offsetsZ[i]
            },
            clawColor,
        ...
        LIBS.rotateZ(this.POSITION_MATRIX, 0.8*side); //atur posisi arah tangan
    ...
    addCone(radius, height, radialSegments, baseCenter, color, dirY = -1) {
        ...
        LIBS.translateY(this.POSITION_MATRIX, 0.3); //buat tinggi tangan di badan
